...
namespace ProcUtil
{
	...
	template<typename T>
	static inline T Read2(HANDLE process, LPCVOID address, std::size_t size = sizeof(T)) { // Had to put the definition here because you can't define generic methods in the cpp file.
		T buffer;
		ReadProcessMemory(process, address, &buffer, size, 0);
		return buffer;
	};

	static inline std::uintptr_t GetPointerAddress(HANDLE process, std::uintptr_t address) { // Had to put the definition here because you can't define inline methods in the cpp file.
		return Read2<std::uintptr_t>(process, reinterpret_cast<LPCVOID>(address));
	}
